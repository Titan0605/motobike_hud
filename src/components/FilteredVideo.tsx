...
export const FilteredVideo = ({ stream, className, enableFilters = true, saturate = 1.06, contrast = 1.04, brightness = 1, sharpen = false, onStats }: FilteredVideoProps) => {
  ...
  useEffect(() => {
    ...
    const updateStats = () => {
      ...
      if (elapsed >= 1000) {
        ...
        onStats?.({ fps, frames: framesRenderedRef.current });
      }
    };
    ...
    if (typeof (video as HTMLVideoElement & { requestVideoFrameCallback?: (callback: () => void) => number }).requestVideoFrameCallback === "function") {
      const requestFrame = () => {
        if (stopped) {
          return;
        }
        ...
        renderFrame();
        videoFrameCallbackRef.current = (video as HTMLVideoElement & { requestVideoFrameCallback: (callback: () => void) => number }).requestVideoFrameCallback(requestFrame);
      };
    ...
    return () => {
      ...
      if (videoFrameCallbackRef.current && typeof (video as HTMLVideoElement & { cancelVideoFrameCallback?: (handle: number) => void }).cancelVideoFrameCallback === "function") {
        (video as HTMLVideoElement & { cancelVideoFrameCallback: (handle: number) => void }).cancelVideoFrameCallback(videoFrameCallbackRef.current);
      }
      gl.deleteTexture(texture);
      gl.deleteBuffer(buffer);
      gl.deleteProgram(program);
    };
  }, [stream, enableFilters, saturate, contrast, brightness, sharpen, onStats]);
